...
/*void DPMaster::WaypointCallBack(const geometry_msgs::PoseStamped::ConstPtr& msg) {
  // 提取 PoseStamped 中的位置信息
...
void DPMaster::WaypointCallBack(const geometry_msgs::PoseStamped::ConstPtr& msg) {
  ...
  if (layer_id != -1) {
    // 计算新的目标点：机器人当前位置和目标点连线的延长线上的无限远点
    // Point3D direction = goal_p - robot_pos_;  // 计算从机器人到目标点的方向向量
    // direction = direction.normalize();  // 将方向向量归一化
    // const float max_distance = 10.0;  // 设定一个“无限远”的距离，可以根据需求调整
    // Point3D goal_p = robot_pos_ + direction * max_distance;  // 延长目标方向，计算新的目标点
    // goal_p.z = robot_pos_.z;
    // 更新目标点和图层 ID
    graph_planner_.UpdateGoal(goal_p, layer_id, false);
    is_goal_update_ = true;

    // 可视化新的目标点
    planner_viz_.VizPoint3D(goal_p, "extended_goal", VizColor::MAGNA, 1.5);

    //ROS_INFO("DPMaster: New goal set to infinity in the direction of the original goal.");
  } else {
    //ROS_WARN("DPMaster: goal layer id assign failed.");
  }
}
