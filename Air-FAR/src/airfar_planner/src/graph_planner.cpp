...
void GraphPlanner::UpdateGraphTraverability(const NavNodePtr& odom_node_ptr) 
{
    ...
    // std::cout << "Current graph contains " << current_graph_.size() << " nodes:" << std::endl;

    // for (const auto& node : current_graph_) {
    //     if (node) { // 确保指针有效
    //         std::cout << "Layer ID: " << node->layer_id
    //                 << ", Position: (" << node->position.x << ", " << node->position.y << ", " << node->position.z << ")"
    //                 << ", G-Score: " << node->gscore
    //                 << ", Is Bottom: " << (node->is_bottom_layer ? "Yes" : "No")
    //                 << std::endl;
    //     } else {
    //         std::cout << "Null node encountered!" << std::endl;
    //     }
    // }
